...
:::confusedDuck
Wait.... if the redirect URI for the client comes from the client and is not generated by the authorization server, then how does the client send it's redirect URI to the authorization server? The connection between the client and the authorization server is not established yet.
:::

:::me
To keep things simple in our example, the redirect URI will be manually entered into the authorization server. It's okay if you don't understand the purpose of the redirect URI right now. We will get to that very soon!
:::
